...
pkgs <- c("tidyverse","here","lme4","broom","units","tidymodels","parallel","cowplot","janitor","sf","RColorBrewer","ggridges","plotly","heatmaply","parsedate","birk","ggthemes","MASS","automap","pbmcapply","ggnewscale","CausalImpact","sjmisc","ggspatial",
...
#New reservess
new_reserves_sf <- st_read("maps/new_reserves/new_reserves_sf.shp") %>%
...
all_reserves = new_reserves_sf %>%
  ...
  mutate(name = ifelse(is.na(name),nom,name)) %>%
  mutate(type = ifelse(is.na(id),"Réserve existante","Nouvelle réserve"))
...
#Creating smaller maps


fishing_effort_plot <- ggplot(data = AIS_fishing_effort,aes(time_range,fishing_effort,color = reserve)) +
  geom_line(size = 1) +
  theme_minimal() +
  scale_color_hp_d(option = "Ravenclaw",labels = c("Pas de réserve","Nouvelles réserves")) +
  labs(x = " ",
...
reserve_legend = c("Banc Capel"="#B45801",
                   "Ride Entrecasteaux" = "#D7AD82",
                   "Nord Walpole" = "#99C2D9",
                   "Nouvelle Hebrides"="#3FA6D9",
                   "Norfolk"="#1081B3",
                   "Fairway"="#046698")
...
heatmap <- ggplot() + 
  ...
  geom_sf(data = all_reserves %>% filter(type != "Parc naturel marin"), aes(fill = type),alpha = 0.5) +
